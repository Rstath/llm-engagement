...
async function renderResearcherDashboard(err = '') {
  ...
  app.innerHTML = `${researcherDashboardStyles()}<h2>Researcher dashboard</h2>${err ? errorBox(err) : ''}
    ...
    <section class="tab-panel" id="tab-stats">
      ...
      <p class="muted">Primary preliminary inference is limited to the three repeated-measures experimental factors: LLM size, personality context, and topic interest. Wilcoxon signed-rank uses participant-level paired means, so conversation rows are not treated as independent participants. Server p-values are preliminary; definitive confidence intervals, rank-biserial effect sizes, and p-values are generated by the offline Sentence-Transformer analysis.</p>
      ${renderStatisticalTests(st)}
      <h3>Big Five × personalization effect — exploratory</h3>
      <p class="muted">The thesis-relevant personality analysis is performed offline: for each participant, personalization benefit is ΔE = mean engagement with personality context − mean engagement without context, then each Big Five trait is related to ΔE using Spearman correlation. General Big Five × mean-engagement correlations are not shown here because they do not directly test personalization benefit.</p>
    </section>

    <section class="tab-panel" id="tab-embeddings">
      <h3>Embedding analysis methodology</h3>
      <div class="metrics-grid">
        ${metricCard('Final model', 'MiniLM-L12-v2', 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2 · offline')}
        ${metricCard('Coherence', 'C', 'Cosine similarity between consecutive turns')}
        ${metricCard('Windowed coherence', 'WC', 'Similarity of a turn to its recent conversational context')}
        ${metricCard('Topic consistency', 'TC', 'Similarity to the initial topic/prompt')}
        ${metricCard('Novelty', 'N', 'Semantic distance from the preceding turn')}
      </div>
      <div class="dashboard-note"><strong>Definitive embedding analysis is offline.</strong> The production server uses lightweight preliminary monitoring metrics only so it remains stable on 512 MB RAM. For the thesis, export raw turns and assignments and run <code>local_final_analysis.py</code>. C, WC, TC and N are embedding-based operational proxies; they are not direct human measures of engagement. The primary composite is .25C + .20WC + .20TC + .15N + .10 turn balance + .10 question rate. Because turn balance has shown no variance, the offline pipeline also reports a pre-declared sensitivity composite with turn balance removed and the remaining weights renormalized.</div>
    </section>
